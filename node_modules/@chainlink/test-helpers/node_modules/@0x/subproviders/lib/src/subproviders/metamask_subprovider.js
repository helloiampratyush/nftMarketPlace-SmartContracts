...
class MetamaskSubprovider extends subprovider_1.Subprovider {
    ...
    handleRequest(payload, _next, end) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (payload.method) {
                ...
                case 'eth_call': {
                    try {
                        const txData = cleanTxDataForChain(yield this._getCommonAsync(), payload.params[0]);
                        const txHash = yield this._web3Wrapper.callAsync(txData);
                        end(null, txHash);
                    }
                    catch (err) {
                        end(err);
                ...
                default:
                    this._provider.sendAsync(payload, (err, response) => {
                        ...
                        if (response.error) {
                            return end(new Error(response.error.message || response.error));
                        }
    ...
    _getCommonAsync() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const chainId = yield this._web3Wrapper.getChainIdAsync();
            return (this._common = chain_utils_1.getCommonForChain(chainId));
        });
    }
}
exports.MetamaskSubprovider = MetamaskSubprovider;
function cleanTxDataForChain(common, rpcData) {
    const txData = web3_wrapper_1.marshaller.unmarshalTxData(rpcData);
